<template>
	<view class="editor-container">
		<view id="fixed-top" class="fixed-top" v-if="readOnly===false"  :class="{ 'isFixed': isFixed }">
			<button class="btn btn-primary" style="margin:5px 10rpx !important"  @click="save">保存</button>
			<button class="btn btn-close text-blue" style="margin:5px 10rpx !important" @click="close">关闭</button>
		</view>
		<view class="fixed-top__place"  v-if="readOnly===false" ></view>
		<scroll-view scroll-y :style="{ height: readOnly?'auto':(scrollViewHeight + 'px' )}">
          ...
          //
			...
			onSuccess(file, res) {
				...
				*/
				...
				if (json.code == 200) {
					this.insertImage(json.data, file)
				} else {
					uni.showToast({
						title: '图片上传失败',
						icon: 'none'
