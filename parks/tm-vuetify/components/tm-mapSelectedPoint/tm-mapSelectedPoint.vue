<template>
	<view class="tm-mapSelectedPoint" :class="[black_tmeme ? 'grey-darken-5' : '']">
		<map
			...
			:show-location="true"
			ref="MapTm"
			@regionchange="moveMapChange"
			:markers="markers"
			:latitude="mapCenter.lat"
			:longitude="mapCenter.lng"
			:style="{ width: `${width}rpx`, height: `400rpx` }"
		></map>
		<view class=" pa-32 ">
			<view class="tm-mapSelectedPoint-contr">
				<view class="pb-32">
					<text class=" text-size-n fulled text-overflow-1">
						当前:{{ adress.adress || '获取失败，请移动地图选择' }}
					</text>
				</view>
				<view class="flex-between pb-10">
					<tm-button @click="confirm" block height="80" style="width:60%">
						确认位置
					</tm-button>
			...
			<view
				...
				class="grey-lighten-5 px-24 round-6 mt-32"
				:class="[black_tmeme ? 'bk grey-darken-4' : '']"
			>
				<scroll-view scroll-y :style="{ height: scrollHeight + 'px' }">
					<view
						@click="selecListitem(item)"
						v-for="(item, index) in adressList"
						:key="index"
						:class="[black_tmeme ? 'bk' : '']"
						class="py-24 border-b-1 flex-between"
					>
						<view class="mr-32">
...
export default {
	...
	computed: {
		...
		color_tmeme: function() {
			if (
				this.$tm.vx.state().tmVuetify.color !== null &&
				this.$tm.vx.state().tmVuetify.color &&
				this.fllowTheme
			) {
	...
	methods: {
				...
				success: async res => {
					if (!res.authSetting['scope.userLocation']) {
						uni.showModal({
							title: '权限受限',
							content:
								'你禁止了位置受限，请打开设置，允许访问地址！否则功能无法使用。',
							cancelText: '我已允许',
							confirmText: '前往设置',
							success: async rks => {
								if (rks.cancel == true) {
		...
		confirm() {
			this.$emit('confirm', {
				...this.adress,
				latitude: this.now_latitude,
				longitude: this.now_longitude
			});
		},
		async getLocation() {
			let t = this;
					...
					uni.getLocation({
						...
						success: async function(res) {
						 const latitude = res.latitude;
							...
							uni.setStorageSync('tmvue_map_selectePoint', {
								latitude: latitude,
								longitude: longitude
							});
						},
			...
			//没有获取到地址资料。通过Ip尝试定位 。
			let adressPoiObj = await uni.$tm.request
				.get('https://apis.map.qq.com/ws/location/v1/ip?key=' + this.mapKey)
				.catch(e => {
					uni.hideLoading();
					console.log('');
					uni.$tm.toast('地址解析错误');
			...
			if (adressPoiObj['status'] == 0) {
				...
				t.markers = [
					t.createMarker(
						adressPoiObj.result.location.lat,
						adressPoiObj.result.location.lng
					)
				];
				await t.poiSidel_byTencentMap(
					adressPoiObj.result.location.lat,
					adressPoiObj.result.location.lng
				);
			}
		...
		async poiSidel_byTencentMap(latitude, longitude) {
			...
			let adressPoiObj = await uni.$tm.request
				.get('https://apis.map.qq.com/ws/geocoder/v1/?' + lot)
				.catch(e => {
					uni.hideLoading();
					uni.$tm.toast('地址解析错误');
				});
			...
			this.$set(this.adress, 'city', [
				adress.address_component.province,
				adress.address_component.city,
				adress.address_component.district
			]);
			this.$emit('change', {
				...this.adress,
				latitude: this.now_latitude,
