<template>
	<view class="tm-upload  ">
		<tm-dialog v-model="showalert" title="视频预览" :showCancel="false" confirmText="关闭预览" confirmColor="primary text">
			<view><video :src="vedioUrl" style="width: 500rpx;height: 300rpx;" objectFit="contain"></video></view>
		</tm-dialog>
		...
		<view class="flex-col">
			<view v-for="(item, index) in list" :key="index" class="text  border-b-2" :class="[color_tmeme, black_tmeme ? 'grey-darken-4 bk ' : '']">
				<view class="flex-between px-24">
					<view class="flex-6 py-18 flex-start  text-overflow text-size-m ">
						...
						<slot name="file" :info="{ item }">{{ item['name'] || item['file_name'] || item['filename'] || item.path }}</slot>
					</view>
					<view v-if="!disabled" class="flex-end flex-6">
						<slot name="right" :info="{ data: item, index: index }">
							...
							<view class="d-inline-block pl-12"><tm-icons @click="del(index)" :black="black_tmeme" name="icon-times" size="32" color="red"></tm-icons></view>
						</slot>
					</view>
				</view>
				<!-- 上传提示语。 -->
				<view
					v-if="tips && !disabled"
					class=" text-size-xs round-b-2 px-24 py-8"
					:class="[
