	...
	<view class="tm-upload flex-start " id="tm-upload">
		...
		>
			<view v-if="!disabled" class="tm-upload-del " :class="[delDirection]">
				<slot name="del"><tm-icons @click="del(index)" :black="black_tmeme" name="icon-times-circle-fill" size="36" color="red"></tm-icons></slot>
			</view>
			<view
				@click.stop="$tm.preview.previewImg(item.url, list, 'url')"
				class="tm-upload-item-ck text flex-center  overflow"
			...
			<view
				v-if="tips && !disabled"
				class="tm-upload-tips text-size-xs round-b-2"
				:class="[
			...
			<!-- 上传的进度。 -->
			<view v-if="item.progress > 0 && item.progress != 100 && !disabled" class="tm-upload-pro green" :style="{ width: item.progress + '%' }"></view>
		</view>

		<view
			@click="addfile"
			v-if="list.length < max && !disabled && showSheet"
			class="tm-upload-item  ma-4 grey-lighten-4 "
			:class="[`round-${round}`]"
			:style="{
				width: itemWidth + 'px',
				height: itemHeight + 'px'
		...
		>
			<view class="tm-upload-item-ck border-a-0 flex-center  text " :class="[color_tmeme, black_tmeme ? 'grey-darken-4 bk' : '', `round-${round}`]">
				<slot name="upload"><tm-icons name="icon-plus" size="36" :color="color_tmeme"></tm-icons></slot>
			</view>
		</view>
	</view>
</template>
