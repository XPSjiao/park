<template>
	<view class="tm-search fulled py-12" :class="[black_tmeme ? 'grey-darken-5' : bgColor, `shadow-${shadow}`]">
		<view class="tm-search-body flex-around">
			<view v-if="showLeft" class="flex-shrink  pl-32 flex-center">
				<slot name="left"><tm-icons @click="$emit('cancel')" :black="black_tmeme" :color="color_tmeme" dense :name="leftIcon"></tm-icons></slot>
			</view>
			<view class="flex-between  fulled py-10  mx-32" :class="[black_tmeme ? ' grey-darken-5' : 'grey-lighten-4', `round-${round}`]">
				<view v-if="prefixText" class="px-16 flex-shrink flex-center text-size-n text-grey">{{ prefixText }}</view>
				<view v-if="prefixIcon" class="pl-16 flex-shrink flex-center"><tm-icons :black="black_tmeme" :color="insertColor_thmeme" dense :name="prefixIcon"></tm-icons></view>
				<input
					@input="input"
					...
					class="flat fulled text-size-n pl-16 py-4 tm-search-body-searc"
				...
				<view v-if="clear && valData.length > 0" class="px-16 flex-shrink flex-center">
					<tm-icons
						...
						:size="24"
						:black="black_tmeme"
						:color="insertColor_thmeme"
						dense
						name="icon-times"
					></tm-icons>
				</view>
				<view v-if="suffixIcon" class="px-16 flex-shrink flex-center">
					<slot name="suffixIcon"><tm-icons @click="confirm" :black="black_tmeme" :color="insertColor_thmeme" dense :name="suffixIcon"></tm-icons></slot>
				</view>
			</view>
			<view @click="confirm" v-if="showRight" class="flex-shrink mr-32 text-size-n" :class="[`text-${color_tmeme}`]">
				<slot name="right">
