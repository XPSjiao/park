<template>
	<view
		class="d-block tm-input overflow"
		:class="[black_tmeme ? (bgTheme ? 'grey-darken-5' : '') : bgTheme, flat ? '' : `px-${padding[0]}`, 'round-' + bgRound, `shadow-${color}-${bgShadow}`]"
	>
		<view :class="(flat ? '' : `  py-${padding[1]} `) + ` ${borderBottom ? (black_tmeme ? 'border-grey-darken-4-b-1 ' : 'border-grey-lighten-4-b-1') : ''}`">
			<view
				@click="onclickInput"
				:class="[vertical ? 'tm-input-col' : 'flex-between ']"
				:style="{
					width: '100%',
					height: 'auto',
			...
			>
				...
				<view v-if="leftIcon || title" class="tm-input-left flex-start flex-shrink" :class="[vertical ? 'pb-24' : '']">
					<!-- icon -->
					<view v-if="leftIcon" class="pr-16 vertical-align-middle flex-center"><tm-icons dense :name="leftIcon" :color="color_tmeme"></tm-icons></view>
					<!-- 标题 -->
					<view v-if="title" class="d-inline-block  " :style="{ fontSize: title_size }" :class="[titleClass, black_tmeme ? 'bk  text-grey-lighten-3' : '']">
				...
				<view class="flex-between " :class="[disabled ? 'opacity-6' : '']" style="width: 100%;">
					...
					<view class="tm-input-center relative fulled">
						<view
							class="flex-start   tm-input-center-wk"
							:class="[
								'round-' + round,
								showIndent ? 'px-16' : '',
								black_tmeme ? (bgColor ? 'grey-darken-4  text-grey-lighten-3' : 'text-grey-lighten-3') : bgColor,
								`text-${textColor}`,
								isFocus && focusShow ? (black_tmeme ? `border-${color_tmeme}-a-1` : `${color_tmeme} outlined `) : ``,
						...
						>
							...
							<input
								...
								:type="inputType"
								:value="value"
								class="tm-input-center-input "
								:class="['text-align-' + align, showError ? 'text-red' : '', ' py-5 ']"
							...
							<textarea
								...
								:class="[maxlength > 0 ? 'pb-46' : '', 'text-align-' + align, showError ? 'text-red' : '', 'pt-16 fulled']"
								:placeholder="placeholder"
							...
							<view v-if="suffixIcon" class="flex-center"><tm-icons :size="26" :name="suffixIcon" :color="suffixIconColor || color_tmeme"></tm-icons></view>
						</view>
						<view
							v-if="maxlength > 0 && inputType == 'textarea'"
							:style="{ bottom: '16rpx', right: '16rpx' }"
							class="tm-input-center-numXz text-align-right text-size-xxs pt-12 text-grey absolute fulled"
						>
							{{ valueLen }}/{{ maxlength }}
						</view>
					</view>
					...
					<view class="tm-input-right flex-end flex-shrink">
						...
						<!-- 后台图标。 -->
						<view v-if="rightIcon" class="pl-10" style="line-height: 0;"><tm-icons dense :name="rightIcon" color="grey-lighten-1"></tm-icons></view>
						<!-- 插入的按钮等内容。 -->
