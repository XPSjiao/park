<template>
	<view class="tm-menu relative d-inline-block fulled">
		<view><slot></slot></view>
		<view v-if="showMenu" class="tm-menu-block absolute animation nowrap" style="width: 90%;" :class="[direction, mentDirection]">
			<view v-if="direction === 'bottom'" class=" px-16" :class="[tipFangx]">
				<text
					class="iconfont icon-sort-up  "
					style="font-size: 46upx;line-height: 0.5;margin-bottom: -4upx;"
					:class="{
			...
			<view class="round-3 overflow-x " :style="{ maxHeight: maxHeight + 'rpx' }" style="margin-left: 37px;">
				<tm-listitem
					@click="
						showMenu = false;
						$emit('change', { index: index, value: item });
					"
					:black="black_tmeme"
					...
					:round="0"
				>
					<text class="text-size-n">{{ listType ? item : item[rangKey] }}</text>
				</tm-listitem>
			</view>
			...
			<view v-if="direction === 'top'" class=" px-16" :class="[tipFangx]">
				<text
					...
					style="font-size: 46upx;line-height: 0.5;margin-top: -4upx;"
					:class="{
						'text-white': !black_tmeme,
						'text-grey-darken-3': black_tmeme
					}"
